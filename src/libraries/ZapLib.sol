...
import {
    SwapParams,
    AddLiquidityParams
} from "../structs/LiquidityStructs.sol";
...
import { IZapLib } from "../interfaces/libraries/IZapLib.sol";
        ...
        for (uint256 i; i < addLiquidityParamsTokensLength; i++) {
            ...
            if (addLiquidityParams.desiredAmounts[i] > 0) {
                ...
                SafeTransferLib.safeApprove(
                    ERC20(addLiquidityParams.tokens[i]), addLiquidityParams.router, 0
                );
                SafeTransferLib.safeApprove(
                    ERC20(addLiquidityParams.tokens[i]),
    ...
    function zapOut(
        ...
        if (zap.removeLiquidityParams.lpToken != address(0)) {
            if (zap.removeLiquidityParams.lpAmountIn > 0) {
                SafeTransferLib.safeApprove(
                    ERC20(zap.removeLiquidityParams.lpToken),
