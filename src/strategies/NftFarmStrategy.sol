...
import {IUniswapV3Pool, IUniswapV3PoolImmutables} from "../interfaces/uniswap/IUniswapV3Pool.sol";

import {StrategyModule, SickleFactory, Sickle} from "../modules/StrategyModule.sol";
import {ConnectorRegistry} from "../ConnectorRegistry.sol";
import {INftFarmConnector} from "../interfaces/INftFarmConnector.sol";
import {INftSettingsRegistry, NftKey} from "../interfaces/INftSettingsRegistry.sol";
import {INftTransferLib} from "../interfaces/libraries/INftTransferLib.sol";
...
import {INftSettingsLib} from "../interfaces/libraries/INftSettingsLib.sol";
import {NftFarmStrategyEvents} from "../events/NftFarmStrategyEvents.sol";
import {INftAutomation} from "../interfaces/INftAutomation.sol";
import {Farm} from "../structs/FarmStrategyStructs.sol";
import {NftPosition, NftDeposit, NftIncrease, NftWithdraw, NftHarvest, NftCompound, NftRebalance, SimpleNftHarvest} from "../structs/NftFarmStrategyStructs.sol";
import {NftSettings} from "../structs/NftSettingsStructs.sol";
import {IERC20} from "lib/openzeppelin-contracts/contracts/interfaces/IERC20.sol";
import {NftZapIn} from "../structs/NftZapStructs.sol";
    ...
    INftAutomation
{
    ...
    modifier nftSupplyUnchanged(INonfungiblePositionManager nft) {
    ...
    function _transfer_in_nft(
        ...
        data[0] = abi.encodeCall(
            INftTransferLib.transferErc721FromUser,
            (nft, tokenId)
        );
    ...
    function _transfer_out_nft(
        ...
        targets[0] = address(nftTransferLib);
        data[0] = abi.encodeCall(
            INftTransferLib.transferErc721ToUser,
            (nft, tokenId)
        );
    ...
    function _deposit_new_nft(
        ...
        targets[0] = farmConnector;
        data[0] = abi.encodeCall(
            INftFarmConnector.depositNewNft,
            (farm, nft, tokenIndex, extraData)
        );
    ...
    function _claim(
        ...
        data[1] = abi.encodeCall(
            IFeesLib.chargeFees,
            (strategyAddress, fee, params.rewardTokens)
        );

        sickle.multicall(targets, data);
    }

    function _compound_nft(Sickle sickle, NftCompound calldata params) private {
        address[] memory targets = new address[](1);
        bytes[] memory data = new bytes[](1);
    ...
    function _get_token_id(
        ...
        return
            IERC721Enumerable(nft).tokenOfOwnerByIndex(
                address(sickle),
    ...
    function _sweep(Sickle sickle, address[] calldata sweepTokens) private {
        ...
        targets[0] = address(transferLib);
        data[0] = abi.encodeCall(
            ITransferLib.transferTokensToUser,
            (sweepTokens)
        );
